...
public class FourQueryGenerator {
	public static void main(String[] args) throws InterruptedException {
		...
		String pw = "pzf";
		//int qTemp[] = {1,2,3,10,11,12,13,14,15,16,17,18};//查询模板          改这
		//int qTemp[] = {2,10,12,13,14,15,16,17,18};
		int qTemp[] = {2,10,13,16,17,18};
		...
		String Date = "190506";//试验日期,试验参数2
		String elabel = "SF10M4";//试验标签,试验参数3
		int runNum =30;//每个线程(即指定特定参数的查询)运行次数,试验参数4
		int paraMax = 1;//允许的最大参数个数,试验参数5
		int MPL=4;//并行程度,试验参数6
		int groupID =190;   //始化组别，可手动修改。试验参数7   改这
		String snapFilePath = "F:\\PZF\\查询计划\\实验\\Rec\\Con4\\";//快照地址,试验参数8
		...
		for(int i = 0;i < qTemp.length;i ++){  //qTemp中第i个query,//外层循环为Query
			for(ii=i+1;ii<qTemp.length;ii++){ 
				for(iii=ii+1;iii<qTemp.length;iii++){
					for(i4=iii+1;i4<qTemp.length;i4++)
					{
					...
					snapFileName4=snapFilePath+elabel+"_"+groupID+"_"+threadID4+"_"+qTemp[i4]+".txt";
					...
					work4=new workGenerator(Date,DBinfo[2][0],DBinfo[2][1],Fraction,User[0][0],pw,User[0][1],elabel,groupID,threadID4,qTemp[i4],0,runNum,snapFileName4,buffer,latch,0); 
