...
public class TwoActiveQueryGenerator {
	public static void main(String[] args) throws InterruptedException {
		String DBinfo[][] = {  //数据库地址，数据库ID
				...
				{"jdbc:postgresql://localhost:5432/postgres","12938"}//12938为数据库id，即DBid，通过select * from pg_stat_statements可以查到
		};
		String User[][] = {//用户名，用户名ID
				{"postgres","10"},{"user2","17135"},{"postgres","17137"}
		};
		String pw = "postgres";
		...
		String elabel = "SF10M2";//试验标签,试验参数3
		...
		String snapFilePath = "E:\\学习\\太原理工大学\\课题\\Snapshot\\";//快照地址,试验参数8
		...
		for(int i = 0;i < qTemp.length;i++){  //qTemp中第i个query,//外层循环为Query
			for(ii=0;ii<qTemp.length;ii++){
				
				snapFileName1 = snapFilePath + elabel +"_"+groupID+"_"+threadID1+"_"+qTemp[i]+".csv";
				snapFileName2=snapFilePath+elabel+"_"+groupID+"_"+threadID2+"_"+qTemp[ii]+".csv";
				...
				latch = new CountDownLatch(MPL);       
				groupID++;
				System.out.println("内层循环第"+ii+"轮结束");
			}
			System.out.println("外层循环第"+i+"轮结束");
